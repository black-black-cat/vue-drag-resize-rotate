...
export default {
  ...
  computed: {
    ...
    cursors () {
      let list = ['n', 'ne', 'e', 'se', 's', 'sw', 'w', 'nw'];
      let ro = this.rotation % 360;
      if (ro < 0) {
        ro += 360;
      }
      let index = Math.floor((ro + 22.5) / 45);
      let changed = [];
      let i = index;
      while (changed.length < 8) {
        i = i % 8;
        changed.push(list[i]);
        i++;
      }
    ...
    drrStick () {
      return (stick) => {
        ...
        stickStyle.cursor = this.cursors[stick] + '-resize';
        return stickStyle;
      };
    },
  },
  ...
  methods: {
    ...
    stickMove (ev) {
      ...
      if (this.currentStick == 'ro') {
        ...
        this.limitSize(this.width, this.height, pn);
    ...
    limitSize (width, height, delta) {
      ...
      function run () {
        let v = new Vector(
          delta.x / Math.abs(delta.x || 1) * Math.abs(width - this.stickStartPos.width) / 2,
          delta.y / Math.abs(delta.y || 1) * Math.abs(height - this.stickStartPos.height) / 2
        ).rotate(Vector.rad(this.rotation));
        this.cx = this.stickStartPos.cx + v.x;
        this.cy = this.stickStartPos.cy + v.y;
        this.width = width;
        this.height = height;
      }
    },
